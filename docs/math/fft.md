...
```
```javascript
双缓冲技术（推荐），但是未验证是否正确，等用到fft海洋才知道是否正确

var<workgroup> sharedData: array<vec2<f32>, 128>;
var<workgroup> tempData: array<vec2<f32>, 128>;

// 初始加载到 sharedData
if (global_id.x < 128) {
  sharedData[global_id.x] = load_initial_data(global_id.x);
}
...
for (var m = 0u; m < 3u; m++) {
  ...
  let step = 1u << m;
  let blockSize = 1u << (m + 1u);
  let blockNum = 128u / blockSize;
  
  // 每个线程处理自己的部分
  for (var n = 0u; n < blockNum; n++) {
    let baseIdx = global_id.x;
    let iterations = blockSize / 2u / workgroupSize.x;
    
    for (var i = 0u; i < iterations; i++) {
      let k = baseIdx + i * workgroupSize.x;
      if (k >= blockSize/2u) continue;
      
      let indexIn = n * blockSize + k;
      let inputData1 = sharedData[indexIn];
      let inputData2 = sharedData[indexIn + step];
      
      let outputIndex1 = 2u * (indexIn - (indexIn % (1u << m))) + (indexIn % (1u << m));
      let outputIndex2 = outputIndex1 + step;
      
      let indexW = k * (128u / blockSize);
      let w = W[indexW];
      
      let p2 = complexMultiply(inputData2, w);
      tempData[outputIndex1] = complexAdd(inputData1, p2);
      tempData[outputIndex2] = complexSub(inputData1, p2);
    }
  }
  ...
  // 交换缓冲区（除了最后一次）
  if (m < 2) {
    for (var i = 0u; i < iterations_per_thread; i++) {
      let idx = global_id.x + i * workgroupSize.x;
      if (idx >= 128) continue;
      sharedData[idx] = tempData[idx];
    }
}
}
