  ...
  import('glslCanvas').then(module => {
    ...
    glslCanvas.load(`
      ...
      void main() {
        ...
        // 想象屏幕是一扇固定的窗口，fbm 噪声是窗口外面的一张无限大布匹。每帧渲染时，屏幕上每个像素都去布匹上"采样"一个点的颜色
        // uv.y - u_time 的效果是：随着时间推移，采样点在布匹上向下移动
        // 时刻 t=0：屏幕底部采 v=0.0，屏幕顶部采 v=1.0
        // 时刻 t=1：屏幕底部采 v=-1.2，屏幕顶部采 v=-0.2   ← 整体往下偏移了
        // 采样窗口在布匹上下移，等价于布匹相对于窗口向上滚动——视觉上看到的内容在向上运动，产生火焰往上燃烧的效果
        // 但是，由于纹理被映射到屏幕上的一个固定区域（这个“固定区域”是重点），这些点在屏幕上的位置实际上会向上移动（因为屏幕上的 y 坐标是向上增加的）
        // 这和 UV 坐标系的方向有关：减小 v 值 = 采样点下移 = 画面内容上移，符合直觉的"负号让东西往上走"
        // 最后，这就造成了视觉上的错觉：纹理看起来像是从下往上移动
        // -------------------------------------------
        // 可以通过以下代码查看相关效果
