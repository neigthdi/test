  ...
  import('glslCanvas').then(module => {
    ...
    glslCanvas.load(`
      ...
      void main() {
        ...
        // -------------------------------------------
        // 屏幕上的噪声纹理是由 fbm 函数生成的噪声值映射到颜色值
        // 当 uv.y 减小时，相当于噪声纹理向下移动，但屏幕上的显示效果是从下往上移动，原因如下：
        //     屏幕上的每个像素点对应一个固定的 uv 坐标
        //     当 uv.y 减小时，新的噪声值会出现在屏幕的下方（uv 坐标较小处）
        //     但由于屏幕是从上到下扫描显示的，当 uv.y 减小时，新的噪声值出现在屏幕下方，但屏幕显示时，新的噪声值会逐渐“填充”到屏幕的上方区域，形成从下往上移动的视觉效果
        // 所以：
        //     噪声的“向下移动” 是指噪声纹理的 uv.y 坐标随时间减小
        //     屏幕“从下往上移动” 是因为屏幕的显示机制是从上到下扫描的，新的噪声值逐渐出现在屏幕上方，形成从下往上的视觉效果
        // 最终的运动是从下往上运动
        // -------------------------------------------
        // 视觉上的“从下往上移动”
