  ...
  import('glslCanvas').then(module => {
    ...
    glslCanvas.load(`
      ...
      void main() {
        ...
        // 当减小uv.y时，纹理上的点会向下移动
        // 但是，由于纹理被映射到屏幕上的一个固定区域（这个“固定区域”是重点），这些点在屏幕上的位置实际上会向上移动（因为屏幕上的y坐标是向上增加的）
        // 这就造成了视觉上的错觉：纹理看起来像是从下往上移动
        // -------------------------------------------
        // 可以通过以下代码查看相关效果
        // 正常情况下，每个颜色在屏幕的视觉中占领 0.2 个单位，但是红色是 -无穷大到 0.2，紫色是 0.8 到 +无穷大
        // 如果 uv.y - 0.1，那么采样点在噪声纹理中向下移动了0.1个单位，此时红色的范围会变大，紫色的范围变小，屏幕（即上面说的“固定区域”）所展示的效果是往上滚动，坐标范围从 0~1 变成 -0.1~0.9
