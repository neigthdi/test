...
// 为什么选择椭球体作为火焰的基础形状？
// 火焰的形状通常是一个向上延伸的、逐渐变细的形状。椭球体（或扁平的椭球体）是一个很好的基础形状，因为它可以近似火焰的整体轮廓。
// 通过调整椭球体的参数（如扁平程度），可以更好地控制火焰的形状。
//-----------------------------------------------------------------------------------------------------------------------
// 为什么使用光线行进算法（Ray Marching）？
// 光线行进算法是一种用于渲染复杂场景（如体积效果、SDF 等）的算法。它通过沿着光线方向逐步前进，计算光线与场景的交点。
// 这种方法非常适合渲染火焰，因为它可以处理火焰的不规则形状和动态变化。
...
const onStart = () => {
  import('glslCanvas').then(module => {
    ...
    glslCanvas.load(`
      ...
      //    vec3(1., 0.5, 1.) 使火焰在 Y 轴方向上更扁平。
      //    vec3(0., -1., 0.) 是火焰的中心位置。
      //    -1.0 是椭球体的半径。
      // 2、noise(p + vec3(0., u_time * 2., 0.))：
      //    添加动态噪声，使火焰随时间变化。
      // 3、noise(p * 3.) * 0.5：
      //    添加更高频率的噪声，用于增加火焰的细节。
      // 4、0.25 * p.y：
