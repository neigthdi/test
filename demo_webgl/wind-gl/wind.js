(function(global, factory) {
	...
	var drawFrag =
		...
		uniform sampler2D u_color_ramp;  //纹理图片像素数据
		varying vec2 v_particle_pos;
		void main() {
			...
			gl_FragColor = texture2D(u_color_ramp, ramp_pos); //采集纹素，逐片元赋值像素值
		}`;
	...
	WindGL.prototype.resize = function resize() {
		...
		this.backgroundTexture = createTexture(gl, gl.NEAREST, emptyPixels, gl.canvas.width, gl.canvas
			.height);
		this.screenTexture = createTexture(gl, gl.NEAREST, emptyPixels, gl.canvas.width, gl.canvas
			.height);
	};
	...
	prototypeAccessors.numParticles.set = function(numParticles) {
		...
		for (var i = 0; i < particleState.length; i++) {
			particleState[i] = Math.floor(Math.random() *
			256); // randomize the initial particle positions
		}
		// textures to hold the particle state for the current and the next frame
		this.particleStateTexture0 = createTexture(gl, gl.NEAREST, particleState, particleRes,
			particleRes);
		this.particleStateTexture1 = createTexture(gl, gl.NEAREST, particleState, particleRes,
			particleRes);
